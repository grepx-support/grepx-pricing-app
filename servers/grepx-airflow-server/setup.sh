#!/bin/bash
# setup.sh - One-time project setup for Apache Airflow
...
# Handle clean option
if [ "${1:-}" = "clean" ]; then
    echo "Cleaning grepx-airflow-server..."
    if [ -d "$VENV_DIR" ]; then
...
echo "Setting up grepx-airflow-server..."
...
echo "grepx-airflow-server setup complete"
